...
const SingleProduct = ({ product, categories }) => {
  ...
  return (
    <>
      <Box key={product.id} marginX='130px' marginTop='120px' marginBottom='178px'>
        <Box
          display='flex'
      ...
      <Box marginX='130px'>
        <Box>
          <Text fontSize={{ base: "lg", md: "2xl", xl: "3xl" }} fontWeight={700}>
            Related Products
          </Text>
        </Box>
        <Box display='flex' flexWrap='wrap' marginLeft='-9px'>
          {categories
            .filter((item, index) => {
              return index < 2;
            })
...
export async function getServerSideProps(context) {
  const productId = context.params.product_slug[context.params.product_slug.length - 1];
